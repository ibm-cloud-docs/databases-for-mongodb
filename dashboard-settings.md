...
**Memory** - If you find that your queries and database activity suffer from performance issues due to a lack of memory, you can scale the amount of RAM allocated to your service. Your MongoDB deployment runs with two members in a cluster, so the amount of memory you allocate to the service is split between both members. Adding memory to the total allocation adds memory to both members equally.

The minimum RAM for a MongoDB deployment is 2048 MB, adjustable in step sizes of 256 MB.  This equates to an initial allocation of 1028 MB per member with 128 MB increments available.
...
{{site.data.keyword.databases-for-mongodb}} is provisioned with a default admin user. You can set or change the the admin password of your deployment by using _Update Password_.
