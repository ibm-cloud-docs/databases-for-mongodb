...
keywords: oplog, operations log, oplogurl
...
To access the MongoDB operations log through `oplogUrl`, create an `oplog` user using the {{site.data.keyword.databases-for-mongodb_full}} `admin` user. The `oplog` user can set `&readPreference=primary` so that reads always happen from the primary.
